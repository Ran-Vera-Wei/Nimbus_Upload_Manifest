...
- `manufacture_name` (≈ BW): if length > 100 → keep first half
- `manufacture_address` (≈ BX): if length > 225 → keep first half
- `Unnamed: 77` (≈ BZ) optional: if length > 8 → keep first half
...
`mawb`: set L2 (`consignee_id_number`) to `2567704`
""")
